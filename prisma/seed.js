...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const wikiModules = [];
        let currentWikiMod = null;
        let currentWikiTopic = null;
        // Parse markdown for premium content
        for (const line of lines) {
            const modMatch = line.match(/^# Module \d+:\s*(.*)/);
            if (modMatch) {
                if (currentWikiTopic && currentWikiMod)
                    currentWikiMod.topics.push(currentWikiTopic);
                if (currentWikiMod)
                    wikiModules.push(currentWikiMod);
        ...
        for (let i = 0; i < FULL_CURRICULUM.length; i++) {
            ...
            const wikiMod = wikiModules.find(w => w.title.toLowerCase().includes(modDef.title.toLowerCase()) ||
